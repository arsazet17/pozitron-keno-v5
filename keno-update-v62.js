  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetch(SOURCE_URL, {
        headers: {
          'user-agent': 'Mozilla/5.0 GitHub-Actions Positron-Keno-v6.2/1.0',
...
async function main() {
  ...
  const previousLatest = stored.filter(validDraw)
    .sort((a, b) => Number(a.draw) - Number(b.draw)).at(-1);
  ...
  console.log(`ПОЗИТРОН v6.2: база ${draws.length}, последний №${latest.draw}`);
}
