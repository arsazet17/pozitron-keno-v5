...
(() => {
  ...
  function candidateHtml(candidate, actual) {
    ...
    return `<div class="cluster-card ${full ? 'cluster-full' : ''}">
      ...
      ${actual ? `<div class="cluster-result ${full ? 'good' : ''}">${full ? '✅ ПОЛНАЯ СБОРКА' : `Совпало ${hits} из ${candidate.length}`}</div>` : ''}
    </div>`;
  }
  ...
  function recordHtml(record, expanded = false) {
    ...
    const fullCount = actual ? record.candidates.filter(c => c.numbers.every(n => actual.balls.includes(Number(n)))).length : 0;
    const body = `<div class="cluster-record-summary"><b>${meta.button} №${record.targetDraw}</b><span>${actual ? `проверен · полных ${fullCount}` : 'ожидает результата'}</span></div>
    ...
    return `<details class="cluster-record"><summary><b>${meta.button} тираж №${record.targetDraw}</b><span>${actual ? `✅ полных сборок: ${fullCount}` : '⏳ ожидает'}</span></summary>${body}</details>`;
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .cluster-card.cluster-full{border-color:#43d77b;background:#123525}.cluster-card-head{display:flex;justify-content:space-between;gap:8px;font-size:13px}
      ...
      .cluster-num.hit{border-color:#43d77b;background:#123a28;color:#c9ffda}.cluster-result{font-size:12px;font-weight:900;margin-top:6px;color:#ffcf82}.cluster-result.good{color:#72df95}
